...
const getText = function (text, width, height, options) {
	...
	while (++i < len) {
		...
		x+=spacing*0.5;
		const charPathC = chToPath(text[len-i-1], Object.assign({x, y}, options));
		const colorC = "rgb("+Math.round(96+Math.random() * 128)+","+Math.round(96+Math.random() * 128)+",0)";
		out.push(`<path fill="${colorC}" d="${charPathC}"/>`);
